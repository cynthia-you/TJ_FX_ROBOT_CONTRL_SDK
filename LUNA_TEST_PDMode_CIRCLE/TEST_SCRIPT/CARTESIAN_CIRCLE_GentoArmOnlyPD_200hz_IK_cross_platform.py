"""
Cartesian circle test with Gento humanoid arm-only SDK IK and formal PD tracking.

Kinematics note:
  Arm kinematics (and dynamics) parameters are always loaded from the robot
  controller via `robot.init_single_arm_config(arm_idx)`. No DH/PNVA/BOUND/
  GRV/MASS/MCP/I constants are hard-coded in this script. Only the selected
  arm is initialized and controlled; body/base/head/lift/hand are untouched.

Requested workflow implemented by this script:
1) Safe connect
2) Clear errors
3) Initialize selected arm kinematics from controller config
4) Enter position mode
5) Move to default joint position
6) Wait 5 seconds
7) Use high precision timer + sleep-until + timing stats
8) Build Cartesian circle around fixed center (xyz only, abc fixed)
9) Solve arm-only IK for points and execute at 200Hz in Gento PD mode
10) MOVLA to first circle point, execute circle, MOVLA back to default
11) Finally switch selected arm to IDLE and release robot connection
...
Terminal
python CARTESIAN_CIRCLE_GentoArmOnlyPD_200hz_IK.py --sweep_kd
Only statics csv and json are given in this mode

if
python CARTESIAN_CIRCLE_GentoArmOnlyPD_200hz_IK.py
...
# ---------------------------------------------------------------------------
# Import PYTHON_SDK from common deployment layouts.
# ---------------------------------------------------------------------------
...
from PYTHON_SDK.GentoRobot import (
    GentoRobot,
    FXLogMask,
    FXObjType,
...
CONFIG_DEFAULTS = {
    ...
    "tag": "gento_arm_only_pd_circle_ik",
    "xcorr_max_lag_s": 0.5,
...
# Sweep parameter list: each tuple is (K[7], D[7]).
PARAM_SWEEP_KD_LIST = [
    ([18.0] * 7, [0.4] * 7),
    ([18.0] * 7, [0.3] * 7),
    ([18.0] * 7, [0.2] * 7),
...
def arm_to_obj_type(arm: str) -> int:
    if arm == "A":
        return FXObjType.OBJ_ARM0
    if arm == "B":
        return FXObjType.OBJ_ARM1
    raise ValueError(f"arm must be 'A' or 'B', got {arm}")


def parse_robot_ip(robot_ip: str) -> Tuple[int, int, int, int]:
    parts = [int(x) for x in str(robot_ip).split(".")]
    if len(parts) != 4 or any(p < 0 or p > 255 for p in parts):
        raise ValueError(f"Invalid robot_ip: {robot_ip}")
    return parts[0], parts[1], parts[2], parts[3]


def check_ret(robot: GentoRobot, ret: int, context: str) -> None:
    if ret != 0:
        try:
            msg = robot._get_operate_error_msg(int(ret))
        except Exception:
            msg = str(ret)
        raise RuntimeError(f"{context} failed: {msg} ({ret})")


def check_kin_ret(robot: GentoRobot, ret: int, context: str) -> None:
    if ret != 0:
        try:
            msg = robot._get_operate_error_msg(int(ret))
        except Exception:
...
def init_arm_kinematics(robot: GentoRobot, arm: str) -> None:
    """Initialize only the selected Gento humanoid arm from controller config.

    Kinematic (and dynamic) parameters are read from the controller via
    `init_single_arm_config`. No DH/PNVA/BOUND/MASS/MCP/I constants are
    hard-coded in this script. Body/base/head/lift/hand are intentionally
    left untouched.
    """
    arm_idx = arm_to_index(arm)
    ret = robot.init_single_arm_config(arm_idx)
    check_kin_ret(robot, ret, f"init_single_arm_config(arm_idx={arm_idx})")
    print(f"[kine] initialized selected arm from controller config, arm={arm}, arm_idx={arm_idx}")
    robot.kine_log_level(FXLogMask.FX_LOG_ERROR_FLAG)


def fk_pose_mm_deg(kine: GentoRobot, arm: str, joints_deg: List[float]) -> Optional[List[float]]:
    fk_mat = kine.forward_kinematics(arm_to_index(arm), [float(v) for v in joints_deg])
    if isinstance(fk_mat, int) or not fk_mat:
        return None
    pose_mm_deg = kine.matrix2xyzabc(fk_mat)
    if not pose_mm_deg:
        return None
...
def estimate_lag_s_cartesian_pca_xcorr(
    t_cmd_s: np.ndarray,
...
def estimate_lag_s_cartesian(
    ...
    # Legacy wrapper kept for compatibility; PCA xcorr is no longer the primary metric.
    return estimate_lag_s_cartesian_pca_xcorr(
        t_cmd_s=t_cmd_s,
...
def estimate_lag_s_cartesian_angular_phase(
    t_cmd_s: np.ndarray,
    cmd_xyz_mm: np.ndarray,
    t_fb_s: np.ndarray,
    fb_xyz_mm: np.ndarray,
    ...
    diagnostics: Dict[str, object] = {
        "method": "cartesian_angular_phase",
        "plane": str(plane),
    ...
    if plane_key not in axis_map:
        diagnostics["error"] = f"invalid plane: {plane}"
        return None, diagnostics

    if float(cycle_s) <= 0.0:
        diagnostics["error"] = "cycle_s must be > 0"
        return None, diagnostics

    t_cmd_s, cmd_xyz_mm = _sanitize_time_and_values(t_cmd_s, cmd_xyz_mm)
    t_fb_s, fb_xyz_mm = _sanitize_time_and_values(t_fb_s, fb_xyz_mm)
    if len(t_cmd_s) < 20 or len(t_fb_s) < 20:
        diagnostics["error"] = "insufficient samples"
        return None, diagnostics

    t_grid_s, dt_s = _build_common_time_grid(t_cmd_s, t_fb_s)
    if dt_s is None or len(t_grid_s) < 20:
        diagnostics["error"] = "failed to build common time grid"
        return None, diagnostics

    cmd_i = _interp_to_grid(t_cmd_s, cmd_xyz_mm, t_grid_s)
    fb_i = _interp_to_grid(t_fb_s, fb_xyz_mm, t_grid_s)
    if cmd_i.ndim != 2 or cmd_i.shape[1] != 3 or fb_i.ndim != 2 or fb_i.shape[1] != 3:
        diagnostics["error"] = "expected cartesian arrays with shape (N, 3)"
        return None, diagnostics

    u_idx, v_idx = axis_map[plane_key]
    cmd_u = cmd_i[:, u_idx]
    cmd_v = cmd_i[:, v_idx]
    fb_u = fb_i[:, u_idx]
    fb_v = fb_i[:, v_idx]

    center_u = float(np.mean(cmd_u))
    center_v = float(np.mean(cmd_v))

    cmd_du = cmd_u - center_u
    cmd_dv = cmd_v - center_v
    fb_du = fb_u - center_u
    fb_dv = fb_v - center_v

    cmd_radius = np.hypot(cmd_du, cmd_dv)
    fb_radius = np.hypot(fb_du, fb_dv)
    cmd_radius_median = float(np.median(cmd_radius)) if len(cmd_radius) > 0 else 0.0
    fb_radius_median = float(np.median(fb_radius)) if len(fb_radius) > 0 else 0.0

    diagnostics.update(
        {
            "center_u_mm": center_u,
            "center_v_mm": center_v,
            "cmd_radius_median_mm": cmd_radius_median,
            "fb_radius_median_mm": fb_radius_median,
        }
    )

    if cmd_radius_median <= 1.0:
        diagnostics["error"] = "command median radius <= 1mm"
        return None, diagnostics
    if fb_radius_median <= 1.0:
        diagnostics["error"] = "feedback median radius <= 1mm"
        return None, diagnostics

    theta_cmd = np.unwrap(np.arctan2(cmd_dv, cmd_du))
    theta_fb = np.unwrap(np.arctan2(fb_dv, fb_du))
    if len(theta_cmd) < 20 or len(theta_fb) < 20:
        diagnostics["error"] = "insufficient phase samples"
        return None, diagnostics

    try:
        omega, _ = np.polyfit(t_grid_s, theta_cmd, deg=1)
    except Exception as exc:
        diagnostics["error"] = f"omega fit failed: {exc}"
        return None, diagnostics

    omega = float(omega)
    expected_omega = float((2.0 * math.pi) / float(cycle_s))
    omega_relative_error = (
        float(abs(abs(omega) - expected_omega) / expected_omega) if expected_omega > 1e-12 else None
    )
    diagnostics.update(
        {
            "omega_fit_rad_s": omega,
    ...
    if abs(omega) <= 1e-6:
        diagnostics["error"] = "fitted omega too small"
        return None, diagnostics

    if omega_relative_error is not None and omega_relative_error >= 0.35:
        diagnostics["warning"] = "fitted omega deviates from expected omega"

    dtheta = np.angle(np.exp(1j * (theta_cmd - theta_fb)))
    delay_samples_s = dtheta / omega

    period_s = float((2.0 * math.pi) / abs(omega))
    delay_samples_s = delay_samples_s - np.round(delay_samples_s / period_s) * period_s

    if float(max_lag_s) > 0.0:
        valid_mask = np.abs(delay_samples_s) <= float(max_lag_s)
        valid_delay_s = delay_samples_s[valid_mask]
    else:
        valid_delay_s = delay_samples_s

    diagnostics["valid_sample_count"] = int(len(valid_delay_s))
    if len(valid_delay_s) == 0:
        diagnostics["error"] = "no delay samples inside max_lag_s"
        return None, diagnostics

    median_delay_s = float(np.median(valid_delay_s))
    abs_dev = np.abs(valid_delay_s - median_delay_s)
    mad = float(np.median(abs_dev))
    if mad > 1e-9:
        robust_sigma = 1.4826 * mad
        robust_mask = abs_dev <= (4.0 * robust_sigma)
        filtered_delay_s = valid_delay_s[robust_mask]
        if len(filtered_delay_s) == 0:
            filtered_delay_s = valid_delay_s
    else:
        filtered_delay_s = valid_delay_s

    diagnostics["filtered_sample_count"] = int(len(filtered_delay_s))
    if len(filtered_delay_s) == 0:
        diagnostics["error"] = "no delay samples after robust filtering"
        return None, diagnostics

    delay_s = float(np.median(filtered_delay_s))
    diagnostics.update(
        {
            "delay_median_ms": float(delay_s * 1000.0),
            "delay_mean_ms": float(np.mean(filtered_delay_s) * 1000.0),
            "delay_p95_abs_ms": float(np.percentile(np.abs(filtered_delay_s), 95) * 1000.0),
        }
    )
    return delay_s, diagnostics
...
def save_trajectory_3d_plot(
    ...
    delay_label: str = "Cartesian angular phase delay",
) -> None:
    ...
    delay_text = f"{str(delay_label)}: {float(cart_delay_ms):.1f} ms" if cart_delay_ms is not None else None
...
def run_postprocess_plots(
    ...
    for ts, q in zip(fb_t_s, fb_q_deg):
        pose_mm_deg = fk_pose_mm_deg(kine, arm, [float(v) for v in q.tolist()])
        if not pose_mm_deg:
            continue
    ...
    cart_lag_phase_s: Optional[float] = None
    cart_lag_pca_s: Optional[float] = None
    cart_delay_method = "cartesian_angular_phase"
    cart_delay_diagnostics: Dict[str, object] = {
        "method": "cartesian_angular_phase",
        "plane": str(circle_plane),
        "cycle_s": float(circle_cycle_s),
        "valid_sample_count": 0,
        "filtered_sample_count": 0,
        "error": "insufficient cartesian samples for delay estimation",
    }
    ...
    if len(cmd_xyz_mm) >= 10 and len(fb_xyz_mm) >= 10:
        cart_lag_phase_s, cart_delay_diagnostics = estimate_lag_s_cartesian_angular_phase(
            t_cmd_s=cmd_t_s,
            cmd_xyz_mm=cmd_xyz_mm,
            t_fb_s=fb_xyz_t_s,
            fb_xyz_mm=fb_xyz_mm,
        ...
        cart_lag_pca_s = estimate_lag_s_cartesian_pca_xcorr(
            t_cmd_s=cmd_t_s,
            cmd_xyz_mm=cmd_xyz_mm,
            t_fb_s=fb_xyz_t_s,
            fb_xyz_mm=fb_xyz_mm,
        ...
        if cart_lag_phase_s is not None:
            cart_lag_s = cart_lag_phase_s
            cart_delay_method = "cartesian_angular_phase"
        elif cart_lag_pca_s is not None:
            cart_lag_s = cart_lag_pca_s
            cart_delay_method = "pca_xcorr_fallback"
            if "warning" in cart_delay_diagnostics:
                cart_delay_diagnostics["warning"] = (
                    f"{cart_delay_diagnostics['warning']}; angular phase unavailable, use PCA fallback"
                )
            else:
                cart_delay_diagnostics["warning"] = "angular phase unavailable, use PCA fallback"
        else:
            cart_delay_method = "unavailable"

        cart_plot_path = os.path.join(result_dir, f"{artifact_prefix}_traj3d_cmd_vs_fb.png")
        delay_label = (
            "Cartesian angular phase delay"
            if cart_delay_method == "cartesian_angular_phase"
            else "Cartesian PCA xcorr delay (fallback)"
        )
    ...
    cart_delay_phase_ms = round(float(cart_lag_phase_s) * 1000.0, 4) if cart_lag_phase_s is not None else None
    cart_delay_pca_ms = round(float(cart_lag_pca_s) * 1000.0, 4) if cart_lag_pca_s is not None else None
    ...
    return {
        ...
        "delay_methods": {
            "cartesian_xyz": "angular_phase_circle",
            "cartesian_xyz_legacy": "xcorr_pca_projection",
        ...
        "delays": {
            "cartesian_xyz_angular_phase_ms": cart_delay_phase_ms,
            "cartesian_xyz_xcorr_pca_ms": cart_delay_pca_ms,
            "cartesian_delay_method": cart_delay_method,
            "cartesian_xyz_xcorr_ms": cart_delay_pca_ms,
            "cartesian_delay_ms": cart_delay_ms,
            "j4_position_fundamental_phase_ms": j4_delay_ms,
        },
        "cartesian_delay_diagnostics": cart_delay_diagnostics,
...
def normalize_cli_path(path_text: object) -> str:
    ...
    relative separators to POSIX separators.

    Windows drive-letter paths such as ``C:\\...`` are intentionally not
    translated on Linux; mount them explicitly, e.g. ``/mnt/c/...`` under WSL.
    """
...
def parse_cpu_affinity(text: object) -> Optional[set]:
    ...
    for part in raw.split(","):
        ...
        if "-" in part:
            ...
            start_cpu, end_cpu = int(start_s), int(end_s)
...
class PlatformTimingTuner:
    ...
    Windows: calls winmm.timeBeginPeriod(1), matching the original Windows-only
    high-precision timer behavior.
    ...
    def start(self) -> None:
        ...
        self._warn("No OS-specific timing tuner for this platform; using perf_counter + sleep_until only.")
...
def print_platform_timing_info(args: argparse.Namespace) -> None:
    info = perf_counter_info()
    print(
        f"[platform] {platform.system()} {platform.release()} | "
        f"Python {platform.python_version()} | perf_counter resolution "
        f"{float(info['resolution_s']) * 1e6:.3f} us"
    )
    if os.name == "nt":
        print("[platform] Windows mode: using winmm.timeBeginPeriod(1) around the timed 200Hz loop.")
...
def resolve_path(path_str: object) -> str:
    p = normalize_cli_path(path_str)
    if not p:
        return p
    if os.name != "nt" and re.match(r"^[A-Za-z]:[\\/]", p):
        raise ValueError(
            f"Windows drive-letter path is not valid on this Linux/POSIX runtime: {p!r}. "
            "Use the mounted POSIX path instead, for example /mnt/c/..."
        )
    if os.path.isabs(p):
        return p
...
def send_joint_command(
    ...
    """Send a joint target to the selected arm only.

    Position mode uses runtime_set_joint_pos_cmd; formal 200 Hz tracking uses
    runtime_set_joint_pos_pd_cmd. No body/base/head/lift/hand command is sent.
    """
    obj_type = arm_to_obj_type(arm)
    q = [float(v) for v in joints_deg]
    if len(q) != 7:
        raise ValueError(f"joints_deg must have 7 values, got {len(q)}")

    if pd_mode:
        ret = robot.runtime_set_joint_pos_pd_cmd(obj_type, q)
        check_ret(robot, ret, "runtime_set_joint_pos_pd_cmd")
    else:
        ret = robot.runtime_set_joint_pos_cmd(obj_type, q)
        check_ret(robot, ret, "runtime_set_joint_pos_cmd")


def safe_connect_and_clear(robot: GentoRobot, dcss: object, robot_ip: str, arm: str) -> None:
    del dcss
    ip1, ip2, ip3, ip4 = parse_robot_ip(robot_ip)
    ret = robot.link(ip1, ip2, ip3, ip4, log_level=FXLogMask.FX_LOG_ERROR_FLAG)
    if not robot._connected:
        raise RuntimeError(f"Robot link failed: {robot._get_operate_error_msg(ret)}")

    time.sleep(0.5)

    # Print SDK and controller version for traceability (best-effort, never blocks startup).
    try:
        print(f"[version] SDK version: {robot.get_sdk_version()}")
        print(f"[version] Controller version: {robot.get_controller_version()}")
    except Exception as exc:
        print(f"[warn] get version failed: {exc}")

    # Only recover the selected arm. Do not switch or command body/base/lift/head/hand.
    obj_type = arm_to_obj_type(arm)
    state = robot.current_state(obj_type)
    if state_map.get(state) == "Error":
        ret, system_errorcode = robot.reset_error(obj_type, 2000)
        if ret != 0:
            raise RuntimeError(f"reset_error({arm}) failed: {error_dict.get(system_errorcode, system_errorcode)}")
        time.sleep(0.2)
    ...
    for _ in range(5):
        ...
        if "error" in rt_dict:
            raise RuntimeError(str(rt_dict["error"]))
        frame_serial = int(rt_dict.get("frame_serial", 0))
        print(f"connect frames: {frame_serial}")
        if frame_serial != 0 and frame_serial != frame_update:
            motion_tag += 1
            frame_update = frame_serial
        # Touch only selected arm feedback to confirm it exists.
        _ = rt_dict["arms"][idx]["fb"]["fb_pos"]
        time.sleep(0.1)
...
def enter_position_mode(robot: GentoRobot, arm: str, vel_ratio: int, acc_ratio: int) -> None:
    """Switch only the selected arm to position mode."""
    obj_type = arm_to_obj_type(arm)
    state = robot.current_state(obj_type)
    if state_map.get(state) == "Error":
        ret, system_errorcode = robot.reset_error(obj_type, 2000)
        if ret != 0:
            raise RuntimeError(f"reset_error({arm}) failed: {error_dict.get(system_errorcode, system_errorcode)}")
    elif state_map.get(state) != "IDLE":
        ret = robot.switch_to_idle(obj_type, 2000)
        check_ret(robot, ret, "switch_to_idle before position")

    ret = robot.switch_to_position_mode(obj_type, 2000, float(vel_ratio), float(acc_ratio))
    check_ret(robot, ret, "switch_to_position_mode")
    time.sleep(0.2)
...
def enter_pd_mode(
    ...
    """Switch only the selected arm to Gento PD mode for cyclic joint commands."""
    obj_type = arm_to_obj_type(arm)
    state = robot.current_state(obj_type)
    if state_map.get(state) == "Error":
        ret, system_errorcode = robot.reset_error(obj_type, 2000)
        ...
        ret = robot.switch_to_idle(obj_type, 2000)
        check_ret(robot, ret, "switch_to_idle before PD")

    cycle_time_ms = max(1, int(round(1000.0 / max(float(ctrl_hz), 1.0))))
    ret = robot.config_set_pd_cmd_cycle_time(cycle_time_ms)
    check_ret(robot, ret, f"config_set_pd_cmd_cycle_time({cycle_time_ms}ms)")

    ret = robot.switch_to_pd_mode(
        obj_type,
        2000,
        float(vel_ratio),
        float(acc_ratio),
        [float(v) for v in joint_k],
        [float(v) for v in joint_d],
    )
    check_ret(robot, ret, "switch_to_pd_mode")
    print(f"[pd] arm={arm}, cycle_time_ms={cycle_time_ms}, K={joint_k}, D={joint_d}")
    time.sleep(0.2)


def get_current_joints(robot: GentoRobot, dcss: object, arm: str) -> List[float]:
    del dcss
    idx = arm_to_index(arm)
    rt_dict = robot.get_rt_dict()
    if "error" in rt_dict:
        raise RuntimeError(str(rt_dict["error"]))
    q = rt_dict["arms"][idx]["fb"]["fb_pos"]
    return [float(v) for v in q]


def get_current_pose_mm_deg(kine: GentoRobot, robot: GentoRobot, dcss: object, arm: str) -> List[float]:
    q_now = get_current_joints(robot, dcss, arm)
    pose_mm_deg = fk_pose_mm_deg(kine, arm, q_now)
...
def wait_until_joint_target_reached(
    ...
    while time.perf_counter() < deadline:
        ...
        if stable_count >= int(max(1, stable_samples)):
            return True, float(last_err)

        time.sleep(max(0.001, float(check_period_s)))

    return False, float(last_err)


def _format_uniform_list(values: List[float]) -> str:
    if len(values) == 7 and all(abs(float(v) - float(values[0])) <= 1e-12 for v in values):
        return f"[{float(values[0]):.1f}]*7"
    return json.dumps([float(v) for v in values], ensure_ascii=False)


def _json_list_str(values: List[float]) -> str:
    return json.dumps([float(v) for v in values], ensure_ascii=False)


def solve_ik_pose_mm_deg(
    kine: GentoRobot,
    arm: str,
    pose_mm_deg: List[float],
    ref_joints_deg: List[float],
) -> Optional[List[float]]:
    sp = FX_InvKineSolverParams()
    pose_mat = kine.xyzabc2matrix([float(v) for v in pose_mm_deg])
    if not pose_mat:
        return None

    pose_mat_1x16 = kine.mat4x4_to_mat1x16(pose_mat)
    for i, v in enumerate(pose_mat_1x16):
        sp.m_Input_IK_TargetTCP[i] = float(v)
    for i, v in enumerate(ref_joints_deg):
        sp.m_Input_IK_RefJoint[i] = float(v)

    ik_result = kine.inverse_kinematics(arm_to_index(arm), sp)
    if isinstance(ik_result, int) or not ik_result:
        return None

    return [float(ik_result.m_Output_RetJoint[i]) for i in range(7)]
...
def plan_movla_points_compat(
    ...
    ret = kine.plan_linear(
        arm_to_index(arm),
        [float(v) for v in start_xyzabc_mm_deg],
        [float(v) for v in end_xyzabc_mm_deg],
    ...
    if isinstance(ret, int):
        check_kin_ret(kine, ret, "plan_linear")

    point_set, num_points = ret
    points: List[List[float]] = []
    for i in range(int(num_points)):
        points.append([float(point_set[i * 7 + j]) for j in range(7)])
    return points


def execute_joint_points(robot: GentoRobot, arm: str, points: List[List[float]], send_hz: float) -> None:
    if not points:
...
def execute_timed_joint_trajectory(
    ...
    timing_tuner = PlatformTimingTuner(
        period_ms=1,
        linux_timer_slack_ns=int(linux_timer_slack_ns),
        linux_rt_priority=int(linux_rt_priority),
        cpu_affinity=cpu_affinity,
    )
    timing_tuner.start()
    hp_timer_started = bool(timing_tuner.enabled)
    ...
    try:
        for i, q_cmd in enumerate(joint_points_deg):
            ...
            send_joint_command(robot, arm, q_cmd, pd_mode=True)
            ...
            try:
                rt_dict = robot.get_rt_dict()
                q_fb = rt_dict["arms"][arm_idx]["fb"]["fb_pos"]
    ...
    timing_tuner_summary = timing_tuner.summary()
    elapsed_s = time.perf_counter() - t0

    return {
        "high_precision_timer_enabled": hp_timer_started,
        "platform_timing_tuner": timing_tuner_summary,
...
def run_postprocess_metrics_only(
    kine: GentoRobot,
    arm: str,
    tracking_data: Dict[str, object],
    max_lag_s: float,
    ...
    try:
        ...
        return {
            "ok": False,
            "test_ok": False,
            "error_message": f"tracking_data parse failed: {exc}",
        }
    ...
    if n_cmd_raw < 10 or n_fb_raw < 10:
        return {
            "ok": False,
            "test_ok": False,
            "error_message": "insufficient tracking samples",
            "samples": {
                "command_raw": int(n_cmd_raw),
                "feedback_joint_raw": int(n_fb_raw),
                "command_post_start": 0,
                "feedback_joint_post_start": 0,
                "feedback_cartesian_valid": 0,
            },
        }
    ...
    if n_cmd < 10 or n_fb < 10:
        return {
            "ok": False,
            "test_ok": False,
            "error_message": "insufficient samples after analysis_start_s filter",
            "samples": {
                "command_raw": int(n_cmd_raw),
                "feedback_joint_raw": int(n_fb_raw),
                "command_post_start": int(n_cmd),
    ...
    final_cart_error_mm: Optional[float] = None
    if len(cmd_xyz_mm) >= 10 and len(fb_xyz_mm) >= 10:
        cart_lag_phase_s, cart_delay_diagnostics = estimate_lag_s_cartesian_angular_phase(
            t_cmd_s=cmd_t_s,
            cmd_xyz_mm=cmd_xyz_mm,
        ...
        if len(t_grid_xyz) >= 10:
            ...
            if len(err_mm) > 0:
                ...
                final_cart_error_mm = float(err_mm[-1])
    ...
    final_joint_error_max_deg: Optional[float] = None
    t_cmd_clean, cmd_q_clean = _sanitize_time_and_values(cmd_t_s, cmd_q_deg)
    t_fb_clean, fb_q_clean = _sanitize_time_and_values(fb_t_s, fb_q_deg)
    t_grid_q, _ = _build_common_time_grid(t_cmd_clean, t_fb_clean)
    if len(t_grid_q) >= 10:
        cmd_q_i = _interp_to_grid(t_cmd_clean, cmd_q_clean, t_grid_q)
        fb_q_i = _interp_to_grid(t_fb_clean, fb_q_clean, t_grid_q)
        final_joint_error_max_deg = float(np.max(np.abs(fb_q_i[-1, :] - cmd_q_i[-1, :])))
    ...
    return {
        ...
        "delays": {
            "cartesian_xyz_angular_phase_ms": float(cart_lag_phase_s) * 1000.0 if cart_lag_phase_s is not None else None,
            "cartesian_xyz_xcorr_pca_ms": float(cart_lag_pca_s) * 1000.0 if cart_lag_pca_s is not None else None,
            "cartesian_delay_method": cart_delay_method,
            "cartesian_xyz_xcorr_ms": float(cart_lag_pca_s) * 1000.0 if cart_lag_pca_s is not None else None,
            "cartesian_delay_ms": float(cart_lag_s) * 1000.0 if cart_lag_s is not None else None,
            "j4_position_fundamental_phase_ms": float(j4_lag_s) * 1000.0 if j4_lag_s is not None else None,
        },
        "cartesian_delay_diagnostics": cart_delay_diagnostics,
        "j4_fundamental_hz_used": float(j4_f0_used_hz) if j4_f0_used_hz is not None else None,
        "cartesian_tracking_error_mm": cart_err_stats,
        "final_joint_error_max_deg": final_joint_error_max_deg,
        "final_cart_error_mm": final_cart_error_mm,
        "feedback_sample_count": int(n_fb),
    }


def _extract_timing_metrics(runtime_result: Dict[str, object]) -> Dict[str, Optional[float]]:
    timing = runtime_result.get("timing_stats", {}) if isinstance(runtime_result, dict) else {}
    command_dt = timing.get("command_dt", {}) if isinstance(timing, dict) else {}
    jitter = timing.get("command_jitter", {}) if isinstance(timing, dict) else {}

    return {
        "command_dt_mean_ms": command_dt.get("mean_ms"),
        "command_dt_std_ms": command_dt.get("std_ms"),
        "command_dt_max_ms": command_dt.get("max_ms"),
        "command_dt_p95_ms": command_dt.get("p95_ms"),
        "jitter_mean_ms": jitter.get("mean_ms"),
        "jitter_std_ms": jitter.get("std_ms"),
        "jitter_max_ms": jitter.get("max_ms"),
        "jitter_p95_ms": jitter.get("p95_ms"),
        "overrun_count": runtime_result.get("overrun_count") if isinstance(runtime_result, dict) else None,
        "negative_sleep_count": runtime_result.get("negative_sleep_count") if isinstance(runtime_result, dict) else None,
    }
...
def _build_sweep_row(
    ...
    timing_metrics: Dict[str, Optional[float]],
    metric_result: Dict[str, object],
    start_time_str: str,
    end_time_str: str,
    duration_s: float,
) -> Dict[str, object]:
    samples = metric_result.get("samples", {}) if isinstance(metric_result, dict) else {}
    delays = metric_result.get("delays", {}) if isinstance(metric_result, dict) else {}
    cart_err = metric_result.get("cartesian_tracking_error_mm", {}) if isinstance(metric_result, dict) else {}

    cartesian_angular = delays.get("cartesian_xyz_angular_phase_ms")
    cartesian_pca = delays.get("cartesian_xyz_xcorr_pca_ms")
    cartesian_method = delays.get("cartesian_delay_method")
    cartesian_delay_main = cartesian_angular if cartesian_angular is not None else cartesian_pca

    return {
        "trial_index": int(trial_index),
        "k_value": float(joint_k[0]) if len(joint_k) > 0 else None,
        "d_value": float(joint_d[0]) if len(joint_d) > 0 else None,
        "cart_k": _json_list_str(joint_k),
        "cart_d": _json_list_str(joint_d),
        "status": str(status),
        "error_message": str(error_message) if error_message else "",
        "cartesian_delay_ms": cartesian_delay_main,
        "cartesian_delay_method": cartesian_method,
        "cartesian_angular_phase_delay_ms": cartesian_angular,
        "cartesian_pca_xcorr_delay_ms": cartesian_pca,
        "j4_delay_ms": delays.get("j4_position_fundamental_phase_ms"),
        "cart_error_mean_mm": cart_err.get("mean_mm"),
        "cart_error_p95_mm": cart_err.get("p95_mm"),
        "cart_error_max_mm": cart_err.get("max_mm"),
        "command_dt_mean_ms": timing_metrics.get("command_dt_mean_ms"),
        "command_dt_std_ms": timing_metrics.get("command_dt_std_ms"),
        "command_dt_p95_ms": timing_metrics.get("command_dt_p95_ms"),
        "command_dt_max_ms": timing_metrics.get("command_dt_max_ms"),
        "jitter_mean_ms": timing_metrics.get("jitter_mean_ms"),
        "jitter_std_ms": timing_metrics.get("jitter_std_ms"),
        "jitter_p95_ms": timing_metrics.get("jitter_p95_ms"),
        "jitter_max_ms": timing_metrics.get("jitter_max_ms"),
        "overrun_count": timing_metrics.get("overrun_count"),
        "negative_sleep_count": timing_metrics.get("negative_sleep_count"),
        "command_raw": samples.get("command_raw"),
        "feedback_joint_raw": samples.get("feedback_joint_raw"),
        "feedback_cartesian_valid": samples.get("feedback_cartesian_valid"),
        "start_time": start_time_str,
        "end_time": end_time_str,
        "duration_s": float(duration_s),
        "final_joint_error_max_deg": metric_result.get("final_joint_error_max_deg") if isinstance(metric_result, dict) else None,
        "final_cart_error_mm": metric_result.get("final_cart_error_mm") if isinstance(metric_result, dict) else None,
        "feedback_sample_count": metric_result.get("feedback_sample_count") if isinstance(metric_result, dict) else None,
        "test_ok": metric_result.get("test_ok") if isinstance(metric_result, dict) else False,
    }


def run_one_kd_trial(
    robot: GentoRobot,
    dcss: object,
    kine: GentoRobot,
    ...
    joint_k: List[float],
    joint_d: List[float],
    q_exec_deg: List[List[float]],
    cart_exec_mm_deg: List[List[float]],
    first_pose_mm_deg: List[float],
    default_joints_deg: List[float],
    result_dir: str,
) -> Dict[str, object]:
    del first_pose_mm_deg
    del default_joints_deg
    ...
    metric_result: Dict[str, object] = {}
    first_q_deg = [float(v) for v in q_exec_deg[0]]

    print(
        f"[trial {trial_index:02d}/{total_trials:02d}] "
        f"cart_k={_format_uniform_list(joint_k)}, cart_d={_format_uniform_list(joint_d)}"
    )

    try:
        enter_position_mode(robot, args.arm, args.pos_vel_ratio, args.pos_acc_ratio)
        send_joint_command(robot, args.arm, first_q_deg)
        reached, last_err = wait_until_joint_target_reached(
            robot=robot,
            dcss=dcss,
            arm=args.arm,
            target_joints_deg=first_q_deg,
            tol_deg=float(args.home_tol_deg),
            stable_samples=int(args.home_stable_samples),
            timeout_s=float(args.home_timeout_s),
        )
        if not reached:
            raise RuntimeError(f"failed to reach first circle point before trial, max_err_deg={last_err:.3f}")
        time.sleep(0.5)

        enter_pd_mode(
            robot=robot,
            arm=args.arm,
            vel_ratio=args.pd_vel_ratio,
            acc_ratio=args.pd_acc_ratio,
        ...
        tracking_data = runtime_result.get("tracking_data", {}) if isinstance(runtime_result, dict) else {}
        metric_result = run_postprocess_metrics_only(
            kine=kine,
            arm=args.arm,
            tracking_data=tracking_data if isinstance(tracking_data, dict) else {},
            ...
            circle_plane=str(args.plane),
            circle_cycle_s=float(args.cycle_s),
        )
        if not bool(metric_result.get("ok")):
            raise RuntimeError(str(metric_result.get("error_message") or "metrics-only postprocess failed"))

        if bool(args.save_trial_json):
            trial_stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            trial_json_path = os.path.join(result_dir, f"kd_trial_{trial_index:02d}_{trial_stamp}.json")
                ...
                json.dump(
                    {
                        ...
                        "postprocess_metrics": metric_result,
                    },
                    f,
                    indent=2,
                    ensure_ascii=False,
        ...
        try:
            ...
            if not reached:
                raise RuntimeError(f"failed to recover first point after trial, max_err_deg={last_err:.3f}")
            time.sleep(0.5)
        except Exception as recover_exc:
            recover_text = f"recover_to_first_failed: {recover_exc}"
            if status == "ok":
                status = "failed"
                error_message = recover_text
            else:
                error_message = f"{error_message} | {recover_text}" if error_message else recover_text

    end_dt = datetime.now()
    end_time_str = end_dt.strftime("%Y-%m-%d %H:%M:%S")
    duration_s = float((end_dt - start_dt).total_seconds())
    timing_metrics = _extract_timing_metrics(runtime_result)
    ...
    if status == "ok":
        print(
            f"[trial {trial_index:02d}/{total_trials:02d} done] "
            f"delay={row.get('cartesian_delay_ms')}, "
            f"mean_err={row.get('cart_error_mean_mm')}, "
            f"p95={row.get('cart_error_p95_mm')}, "
            f"max={row.get('cart_error_max_mm')}"
        )
    else:
        print(
            f"[trial {trial_index:02d}/{total_trials:02d} done] "
...
def save_kd_sweep_summary(
    summary_rows: List[Dict[str, object]],
    result_dir: str,
    run_stamp: str,
) -> Tuple[str, str]:
    csv_path = os.path.join(result_dir, f"kd_sweep_summary_{run_stamp}.csv")
    json_path = os.path.join(result_dir, f"kd_sweep_summary_{run_stamp}.json")
    ...
    fieldnames = [
        ...
        "error_message",
        "cartesian_delay_ms",
        "cartesian_delay_method",
        "cartesian_angular_phase_delay_ms",
        "cartesian_pca_xcorr_delay_ms",
        "j4_delay_ms",
        "cart_error_mean_mm",
        "cart_error_p95_mm",
        "cart_error_max_mm",
        ...
        "command_raw",
        "feedback_joint_raw",
        "feedback_cartesian_valid",
        "start_time",
        "end_time",
        "duration_s",
        "final_joint_error_max_deg",
        "final_cart_error_mm",
...
def print_sweep_top5(summary_rows: List[Dict[str, object]]) -> None:
    ok_rows = [
        r
        for r in summary_rows
        if str(r.get("status", "")).lower() == "ok" and r.get("cart_error_mean_mm") is not None
    ]
    if not ok_rows:
        print("==== Top 5 by mean Cartesian error ====\n(no successful trial)")
        print("==== Top 5 by max Cartesian error ====\n(no successful trial)")
        return

    by_mean = sorted(ok_rows, key=lambda r: float(r.get("cart_error_mean_mm")))[:5]
    by_max = sorted(
        [r for r in ok_rows if r.get("cart_error_max_mm") is not None],
        key=lambda r: float(r.get("cart_error_max_mm")),
    )[:5]

    print("==== Top 5 by mean Cartesian error ====")
    print("rank, trial_index, K, D, delay_ms, mean_mm, p95_mm, max_mm")
    for rank, row in enumerate(by_mean, start=1):
        print(
            f"{rank}, {row.get('trial_index')}, {row.get('k_value')}, {row.get('d_value')}, "
            f"{row.get('cartesian_delay_ms')}, {row.get('cart_error_mean_mm')}, "
            f"{row.get('cart_error_p95_mm')}, {row.get('cart_error_max_mm')}"
        )

    print("==== Top 5 by max Cartesian error ====")
    print("rank, trial_index, K, D, delay_ms, mean_mm, p95_mm, max_mm")
    for rank, row in enumerate(by_max, start=1):
        print(
            f"{rank}, {row.get('trial_index')}, {row.get('k_value')}, {row.get('d_value')}, "
            f"{row.get('cartesian_delay_ms')}, {row.get('cart_error_mean_mm')}, "
            f"{row.get('cart_error_p95_mm')}, {row.get('cart_error_max_mm')}"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Cartesian circle test with IK, timing control, and arm-only PD formal run")
    ...
    # Kinematics is always loaded from the controller via init_single_arm_config(arm_idx).
    # No --kine_init option is provided; manual M3 hard-coded parameters are intentionally removed.

    parser.add_argument("--default_joints", nargs=7, type=float, default=None, help="Default joints in deg")
    parser.add_argument("--pos_vel_ratio", type=int, default=int(CONFIG_DEFAULTS["pos_vel_ratio"]), help="Position mode velocity ratio")
    parser.add_argument("--pos_acc_ratio", type=int, default=int(CONFIG_DEFAULTS["pos_acc_ratio"]), help="Position mode acceleration ratio")
    parser.add_argument("--pd_vel_ratio", type=int, default=int(CONFIG_DEFAULTS["pd_vel_ratio"]), help="PD mode velocity ratio")
    parser.add_argument("--pd_acc_ratio", type=int, default=int(CONFIG_DEFAULTS["pd_acc_ratio"]), help="PD mode acceleration ratio")
    ...
    parser.add_argument(
        ...
        help="Joint stiffness K for Gento PD mode",
    )
    parser.add_argument(
        "--joint_d",
        nargs=7,
        ...
        help="Joint damping D for Gento PD mode",
    )
    parser.add_argument(
        "--cart_k",
        nargs=7,
        type=float,
        default=None,
        help="Alias of --joint_k (kept for sweep framework compatibility)",
    )
    parser.add_argument(
        "--cart_d",
        nargs=7,
        type=float,
        default=None,
        help="Alias of --joint_d (kept for sweep framework compatibility)",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0005,
        help="Busy-wait threshold used by sleep_until; larger values reduce sleep jitter but use more CPU",
    )
    parser.add_argument(
        "--linux_timer_slack_ns",
        type=int,
        default=50000,
        help="Linux only: best-effort PR_SET_TIMERSLACK value in ns; use -1 to disable",
    )
    parser.add_argument(
        "--linux_rt_priority",
        type=int,
        default=0,
        help="Linux only: request SCHED_FIFO priority 1-99; 0 disables",
    )
    parser.add_argument(
        "--cpu_affinity",
        type=str,
        default="",
        help="Linux only: optional CPU affinity, e.g. '2' or '2,3' or '2-3'",
    )

    parser.add_argument(
        "--sweep_kd",
        action="store_true",
        help="Enable KD parameter sweep mode (no plot output; summary CSV/JSON only)",
    )
    parser.add_argument(
        "--save_trial_json",
        action="store_true",
        help="In sweep mode, save per-trial raw runtime + metrics JSON",
    )
    parser.add_argument(
        "--continue_on_trial_failure",
        action="store_true",
        help="In sweep mode, continue next KD trial when one trial fails",
    )
...
def main() -> int:
    ...
    j4_fundamental_hz = float(args.j4_fundamental_hz) if args.j4_fundamental_hz is not None else (1.0 / float(args.cycle_s))
    single_run_k = [float(v) for v in (args.cart_k if args.cart_k is not None else args.joint_k)]
    single_run_d = [float(v) for v in (args.cart_d if args.cart_d is not None else args.joint_d)]

    result_dir = resolve_path(args.result_dir)
    os.makedirs(result_dir, exist_ok=True)

    print("[step1] initialize GentoRobot SDK")
    robot = GentoRobot()
    kine = robot
    dcss = None
    ...
    try:
        ...
        print("[step2] initialize selected arm-only kinematics from controller config")
        init_arm_kinematics(robot, args.arm)

        print("[step3] enter selected arm position mode")
        enter_position_mode(robot, args.arm, args.pos_vel_ratio, args.pos_acc_ratio)
        ...
        for i, pose_mm_deg in enumerate(circle_poses_mm_deg):
            q_sol = solve_ik_pose_mm_deg(kine, args.arm, pose_mm_deg, ref_j)
            if q_sol is None:
        ...
        # Reuse the same executed trajectory for all sweep trials.
        ...
        if args.sweep_kd:
            ...
            summary_rows: List[Dict[str, object]] = []
            total_trials = len(PARAM_SWEEP_KD_LIST)
            print(f"[sweep] KD sweep enabled, total_trials={total_trials}")

            for trial_index, (joint_k, joint_d) in enumerate(PARAM_SWEEP_KD_LIST, start=1):
                trial_result = run_one_kd_trial(
                    robot=robot,
                    dcss=dcss,
                    kine=kine,
                    args=args,
                    ...
                    joint_k=[float(v) for v in joint_k],
                    joint_d=[float(v) for v in joint_d],
                    q_exec_deg=q_exec_deg,
                    cart_exec_mm_deg=cart_exec_mm_deg,
                    first_pose_mm_deg=first_pose_mm_deg,
                    default_joints_deg=default_joints_deg,
                    result_dir=result_dir,
                )

                row = trial_result.get("row")
                if isinstance(row, dict):
                    summary_rows.append(row)

                trial_status = str(trial_result.get("status", "")).lower()
                if trial_status != "ok" and not bool(args.continue_on_trial_failure):
                    print("[sweep] stop on trial failure (set --continue_on_trial_failure to continue)")
                    break

            csv_path, json_path = save_kd_sweep_summary(
                summary_rows=summary_rows,
                result_dir=result_dir,
                run_stamp=run_stamp,
            )
            print(f"[sweep] summary csv: {csv_path}")
            print(f"[sweep] summary json: {json_path}")
            print_sweep_top5(summary_rows)
        else:
            print("[step6] hot switch selected arm to Gento PD mode (formal circle run)")
            enter_pd_mode(
                robot=robot,
                arm=args.arm,
                vel_ratio=args.pd_vel_ratio,
            ...
            result = {
                ...
                "config": {
                    ...
                    "arm": args.arm,
                    "kine_source": "controller_config_init_single_arm_config",
                    "default_joints_deg": default_joints_deg,
                    "center_xyz_m": center_xyz_m,
                    "abc_deg": abc_deg,
                    "radius_m": float(args.radius_m),
                    ...
                    "linux_timer_slack_ns": int(args.linux_timer_slack_ns),
                    "linux_rt_priority": int(args.linux_rt_priority),
                    "cpu_affinity": str(args.cpu_affinity or ""),
                },
                "runtime": runtime_result,
            ...
            cart_delay_ms = post_delays.get("cartesian_xyz_angular_phase_ms")
            cart_pca_delay_ms = post_delays.get("cartesian_xyz_xcorr_pca_ms")
            cart_delay_method = post_delays.get("cartesian_delay_method")
            j4_delay_ms = post_delays.get("j4_position_fundamental_phase_ms")
            print(
                "[analysis] cartesian_xyz_angular_phase_delay(ms): "
                f"{float(cart_delay_ms):.4f}"
                if cart_delay_ms is not None
                else "[analysis] cartesian_xyz_angular_phase_delay(ms): None"
            )
            print(
                "[analysis] cartesian_xyz_pca_xcorr_delay(ms): "
                f"{float(cart_pca_delay_ms):.4f}"
                if cart_pca_delay_ms is not None
                else "[analysis] cartesian_xyz_pca_xcorr_delay(ms): None"
            )
            print(f"[analysis] cartesian_delay_method: {cart_delay_method}")
            print(
        ...
        default_pose_mm_deg = fk_pose_mm_deg(kine, args.arm, default_joints_deg)
        if default_pose_mm_deg is None:
        ...
        reached_home, last_err_home = wait_until_joint_target_reached(
            robot=robot,
            dcss=dcss,
            arm=args.arm,
            target_joints_deg=default_joints_deg,
        ...
        if reached_home:
            print(f"[shutdown] back to default reached, max|error|={last_err_home:.3f} deg")
        else:
            print(f"[shutdown] back to default timeout, max|error|={last_err_home:.3f} deg")
        returned_to_default = True

        if args.sweep_kd:
            print("[done] sweep completed")
        else:
            print("[done] single trial completed")
        ...
        if connected and not returned_to_default:
            try:
                enter_position_mode(robot, args.arm, args.pos_vel_ratio, args.pos_acc_ratio)
                send_joint_command(robot, args.arm, default_joints_deg)
                reached, last_err = wait_until_joint_target_reached(
                    robot=robot,
                    dcss=dcss,
                    arm=args.arm,
                ...
                if reached:
                    print(f"[shutdown] back to default reached, max|error|={last_err:.3f} deg")
                else:
                    print(f"[shutdown] back to default timeout, max|error|={last_err:.3f} deg")
            except Exception as exc:
                print(f"[shutdown] best-effort return default failed: {exc}")

        # step10: switch only selected arm to IDLE and release SDK resources.
        try:
            ret = robot.switch_to_idle(arm_to_obj_type(args.arm), 2000)
            if ret != 0:
                print(f"[shutdown] switch_to_idle failed: {robot._get_operate_error_msg(ret)}")
            else:
                print(f"[shutdown] selected arm {args.arm} switched to IDLE")
            time.sleep(0.2)
        except Exception as exc:
            print(f"[shutdown] switch_to_idle failed: {exc}")
        ...
        try:
            ...
            print(f"[shutdown] unlink failed: {exc}")
