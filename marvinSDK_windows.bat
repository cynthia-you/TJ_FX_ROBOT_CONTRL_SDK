...
cd contrlSDK100343001 && g++ *.cpp -Wall -O2 -shared -o libMarvinSDK.dll -lws2_32 -lwinmm -DCMPL_WIN && cd ..
cd kinematicsSDK && g++ *.cpp -Wall -O2 -fPIC -shared -o libKine.dll && cd ..
copy /Y contrlSDK100343001\libMarvinSDK.dll "DEMO_C++\" >nul
copy /Y kinematicsSDK\libKine.dll "DEMO_C++\" >nul
echo ✓ c++ finished


echo compile SDK_PYTHON...
del /F /Q contrlSDK100343001\libMarvinSDK.dll 2>nul
del /F /Q kinematicsSDK\libKine.dll 2>nul
cd contrlSDK100343001 && g++ *.cpp -Wall -O2 -shared -o libMarvinSDK.dll -DBUILDING_DLL -D_WIN32 -DCMPL_WIN -fPIC -static -static-libgcc -static-libstdc++ -lws2_32 -lwinmm && cd ..
...
copy /Y contrlSDK100343001\libMarvinSDK.dll SDK_PYTHON\ >nul
