...
//	double trajCondation[10] = { 0 };// -90, -91, 1, 0, 5, 4, 95, 94, 20, -20
    double trajCondation[10] = { 1, 0, 1, 0, 5, 4, 95, 94, 20, -20 };// -90, -91, 1, 0, 5, 4, 95, 94, 20, -20
//	for (int i = 0; i < 10; i++) {
//		trajCondation[i] = stod(vecCon.at(i));
//	}
	//采样周期
	...
	double dtemp = 0;
	int i;
	int LoadDataNum = static_cast<int>(vecLoadData.at(DataTag).size());//所有采集的数据行数
	...
	if (1 == isNoLoad) {
		for (int i = 0; i < 7; i++) {
			tor->torNoLoad[i] = tor7[i]; //cout << tor7[i] << endl;
...
bool OnCalLoadDynamic_7_MarvinSRS(LoadDynamic* loadDyn, const char *userPath)
{
	...
	//--计算负载参数
	double tor4, tor6;
	const double calErr = 0;//补偿计算动态测量与固定位置测量的关节扭矩误差
	...
	//--负载:质心x,y
	double r = 0, theta = 0;
	double tor7_1, tor7_2;//collect data
...
//	loadDyn->inertia[0] = ixx;
//	loadDyn->inertia[1] = iyy;
//	loadDyn->inertia[2] = izz;
...
bool OnCalLoadTor_7_MarvinCCS(LoadTor* tor, int isNoLoad, const char *userPath)
{
	...
	string strLoadDataLine;
	vector<vector<string>> vecLoadData(9);//tag-4-5-6-7-4-5-6-7
	while (getline(ifLoadFile, strLoadDataLine)) {
	...
	//可调参数
	const double torTag[7] = { 0, 1111, 2222, 3333, 4444, 5555, 6666 };
	double trajCondation[10] = { 1, -1, 1, 0, 5, 4, 95, 94, 20, -20 };// -90, -91, 1, 0, 5, 4, 95, 94, 20, -20
//	for (int i = 0; i < 10; i++) {
//		trajCondation[i] = stod(vecCon.at(i));
//	}
	//采样周期
	...
	double dtemp = 0;
	int i;
	int LoadDataNum = static_cast<int>(vecLoadData.at(0).size());//所有采集的数据行数
	int trajCalTimes = 0;//计算正反两段数据
	double tor7[7] = { 0 };
...
bool OnCalLoadDynamic_7_MarvinCCS(LoadDynamic* loadDyn, const char *userPath)
{
	...
	loadDyn->mr[1] = rf[1];
	loadDyn->mr[2] = rf[2];
//	loadDyn->inertia[0] = ixxf;
//	loadDyn->inertia[1] = iyyf;
//	loadDyn->inertia[2] = izzf;
