...
extern "C" bool AxisPln_OnMovL(FXSpln spln, long RobotSerial, double start_pos[6], double end_pos[6], double ref_joints[7], double vel, double acc, double jerk, char* path)
{
	CAxisPln* obj = static_cast<CAxisPln*>(spln);
	if (obj)
	{
		return obj->OnMovL(RobotSerial,ref_joints,start_pos, end_pos, vel, acc, jerk, path);
	}
}

//void AxisPln_OnMovL_KeepJ(FXSpln spln, long RobotSerial, double start_pos[6], double end_pos[6], double vel, char* path);
extern "C" bool AxisPln_OnMovL_KeepJ(FXSpln spln, long RobotSerial, double start_pos[6], double end_pos[6], double vel, char* path)
{
	CAxisPln* obj = static_cast<CAxisPln*>(spln);
	if (obj)
	{
		return obj->OnMovL_KeepJ(RobotSerial, start_pos, end_pos, vel, path);
	}
}
