...
typedef enum
{
	...
	DCSS_CMD_ARM0_GET_DATA_JOINTSTIFF = 119,
	DCSS_CMD_ARM0_GET_DATA_CARTSTIFF = 120, 
	...
	DCSS_CMD_ARM1_GET_DATA_JOINTSTIFF = 219,
	DCSS_CMD_ARM1_GET_DATA_CARTSTIFF = 220, 
	DCSS_CMD_ARM1_SetEstVel_Steps = 221,
	DCSS_CMD_RET_REQUES = 251,

	DCSS_CMD_GET_DATA_GYRO_ACC = 300,
	DCSS_CMD_GET_DATA_GYRO_ANGLE = 301,
	DCSS_CMD_GET_DATA_GYRO_OMEG = 302,

} DCSSCmdType;

typedef struct
{
	double fxDir[6]; // 六维力方向  这里的位置方向为相对于机械臂的基坐标系   姿态为相对于末端坐标系
	double K;		 // 位置方向的刚度
	double F;		 // 沿着给定方向的力
	double FreeDis;	 // 位置方向无力区间   mm
	double Dis;		 // 沿着给定方向的运动距离  mm
	double Kn;		 // 姿态方向的刚度
	double Tn;		 // 姿态方向的扭矩
	double NFreeDis; // 姿态方向的无力区间  度
	double Ndis;	 // 姿态方向的运动距离  度
} FTCmd;
