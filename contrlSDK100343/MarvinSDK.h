...
extern "C"
{
	...
	FX_DLL_EXPORT int CheckSDKTypeCompat(int *pByteOrder);
	////////////////////////////////////////////////////////////////////////////////////////////////
	// 连接机器人
	FX_DLL_EXPORT bool OnLinkTo(unsigned char ip1, unsigned char ip2, unsigned char ip3, unsigned char ip4);
	...
	FX_DLL_EXPORT void EStop(const char *arm);

	// 指定关节伺服软复位, arm: "A" "B"  两种字符是许可值; axis:0~6
	FX_DLL_EXPORT void ServoReset(char arm, int axis);
	...
	FX_DLL_EXPORT bool SetTool(char arm, double kinePara[6], double dynPara[10]);

	// 设置指定手臂的速度和加速度和位置模式,注意PVT和拖动不受该速度限制。arm:"A" "B"  两种字符是许可值; velRatio:0~100; AccRatio:0~100
	FX_DLL_EXPORT bool SetJointMode(char arm, int velRatio, int AccRatio);

	// 设置指定手臂的速度和加速度和关节阻抗模式。arm:"A" "B"  两种字符是许可值; velRatio:0~100; AccRatio:0~100; K:非负值； D：0~1
	FX_DLL_EXPORT bool SetImpJointMode(char arm, int velRatio, int AccRatio, double K[7], double D[7]);
	...
	FX_DLL_EXPORT bool SetImpCartMode(char arm, int velRatio, int AccRatio, double K[7], double D[7], int RotType, double CartCtrlPara[7]);

	// 设置指定手臂的关节空间位置指令（位置模式扭矩模式下的关节指令）。  arm:"A" "B"  两种字符是许可值； joint：七个关节的目标角度(单位：度）
	FX_DLL_EXPORT bool SetJointPostionCmd(char arm, double joint[7]);

	// 设置指定手臂的力控参数和力阻抗模式。arm:"A" "B"  两种字符是许可值; fxDir：任意定义方向； fcAdjLmt：力的调节范围，单位毫米
	FX_DLL_EXPORT bool SetImpForceMode(char arm, double fxDir[6], double fcAdjLmt);
	// 设置指定手臂的力值：arm:"A" "B"  两种字符是许可值; force: 力，单位：牛
	FX_DLL_EXPORT bool SetForceCmd(char arm, double force);

	// 关节空间规划初始化，只需初始化一次
	FX_DLL_EXPORT bool PlnInit(char *path);
	// 关节空间下从当前点规划方式运行到目标点
	FX_DLL_EXPORT bool RunPlnJoint(char arm, double start_joints[7], double stop_joints[7], double vel_ratio, double acc_ratio);
	// 笛卡尔空间下从当前点规划方式运行到目标点，规划点位pset由KinematicsSDK计算接口FX_Robot_PLN_MOVLA计算得出。
	FX_DLL_EXPORT bool RunPlnCart(char arm, void *pset);
	// 中断规划运行，笛卡尔空间和关节空间都适用
	FX_DLL_EXPORT bool StopPln(char arm);

	// 上传本地PVT轨迹文件存为指定ID, arm:"A" "B"  两种字符是许可值; local_file:相对或绝对路径； serial:0~99
	FX_DLL_EXPORT bool SendPVT(char arm, char *local_file, long serial);
	// 设置指定手臂的PVT号并立即运行该轨迹, arm:"A" "B"  两种字符是许可值； id：0~99(SendPVT上传的pvt文件的serial).特别注意运行PVT，需要将机器人位置调到PVT规划轨迹的起点
	FX_DLL_EXPORT bool RunPVT(char arm, int id);

	// 拖动，每种拖动使用完毕需要退出拖动再切换为别的拖动模式，否则拖动效果是叠加混乱的哦。
	// 设置指定手臂为关节拖动。 arm:"A" "B"  两种字符是许可值
	FX_DLL_EXPORT bool SetJointDrag(char arm);
	// 设置指定手臂为笛卡尔拖动。 arm:"A" "B"  两种字符是许可值; type: "X" "Y" "Z" "R" 四种字符是许可值（X/Y/Z/旋转， 四个方向选一）;
	FX_DLL_EXPORT bool SetCartDrag(char arm, char type);
	// 设置指定手臂退出拖动。arm:"A" "B"  两种字符是许可值
	FX_DLL_EXPORT bool ExitDrag(char arm);

	// 手臂末端安装工具的通讯
	// 清缓存数据。arm:"A" "B"  两种字符是许可值
	FX_DLL_EXPORT bool ClearChData(char arm);
	// 获取指定手臂指定通道的数据. arm:"A" "B" 两种字符是许可值; ret_ch==1: CAN/CANFD  ret_ch==2: COM1  ret_ch==3: COM2
	FX_DLL_EXPORT long GetChData(char arm, unsigned char data_ptr[256], long *ret_ch);
	// 给指定手臂指定通道发送数据. arm:"A" "B" 两种字符是许可值; ret_ch==1: CAN/CANFD  ret_ch==2: COM1  ret_ch==3: COM2
	FX_DLL_EXPORT long SetChData(char arm, unsigned char data_ptr[256], long size_int, long set_ch);
	...
	FX_DLL_EXPORT bool Disable(char arm);
