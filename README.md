...
### 关节阻抗模式（Joint Impedance Mode）
    在关节空间内建立力矩与位置偏差的动态关系，表现为“弹簧-阻尼”特性。用户可分别设置每个关节的刚度（范围 0~22， 单位N*m/deg）和阻尼（>0），刚度越高关节“越硬”，阻尼越大，物体振幅减小越快，但对力、位移的响应迟缓，运动时感觉阻力大，有粘滞感； 阻尼越小，减震效果减弱，但运动阻力小，更流畅，停止到位置时有余震感。该模式适用于需要关节级柔顺性的装配、抛光和避障任务，可吸收冲击并适应不规则表面。
